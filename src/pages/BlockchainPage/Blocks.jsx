    ...
    <div className="px-4 md:px-4 lg:px-4 xl:px-12 2xl:px-12">
      ...
      <div className=" flex flex-col space-y-6 md:space-y-0 md:flex-row justify-around ">
        <div className="w-full md:w-[32%]  bg-[#fff7e0] shadow-lg rounded-2xl p-5 ">
          ...
          <div className=" w-full flex flex-row justify-between pt-9 pl-1 ">
            <div>
              <p className=" font-bold md:text-lg text-xl">
                {data1?.totalBlocks &&
                  formatNumberWithCommas(data1?.totalBlocks)}
              </p>
              <p className="pt-4 text-sm text-light-gray">Latest</p>
            </div>

            <div>
              <p className="md:text-lg text-xl font-bold text-dark-red">
                +{data1?.totalBlocks24H && data1?.totalBlocks24H}
              </p>
        ...
        <div className="w-full md:w-[32%]  rounded-2xl p-5 bg-[#cfdeed] shadow-md ">
          ...
          <div className=" w-full flex flex-row justify-between pt-6 pl-1">
            <div>
              <p className="md:text-lg text-xl font-bold">
                {formatNumberWithCommas(
                  Number(data1?.totalBlockReward ?? 0).toFixed(1)
                )}{" "}
                POX
              </p>
            ...
            <div>
              <p className="md:text-lg text-xl text-dark-red font-bold">
                {data1?.totalBlockRewards24h && data1?.totalBlockRewards24h}TRX
              </p>
        ...
        <div className="w-full md:w-[32%] rounded-2xl p-5 bg-[#edd8dc] shadow-md ">
          ...
          <div className=" w-full flex flex-row justify-between pt-8 pl-1">
            <div>
              <p className="md:text-lg text-xl font-bold">
                {formatNumberWithCommas(
                  Number(data1?.burnedPOX ?? 0).toFixed(3)
                )}{" "}
                POX
              </p>
            ...
            <div>
              <p className="md:text-lg text-xl text-dark-red font-bold">
                {data1?.burnedPOX24H && data1?.burnedPOX24H.toFixed(3)} TRX
              </p>
      ...
      <div className="bg-white  rounded-2xl p-2 md:p-7  overflow-x-auto ">
        ...
        <div className="flex flex-row justify-evenly  p-2 pb-3 bg-lightest-gray rounded-lg min-w-[1500px] ">
          ...
          <p className=" w-[16%] font-bold text-center  whitespace-nowrap ">
            {" "}
        ...
        {data?.apiResult &&
          data?.apiResult.map((stablecoin, index) => {
            return (
              <>
                <div
                  className="flex flex-row justify-between border-b-2 p-4 border-text-bg-gray min-w-[1500px]"
                  key={index}
                >
                  <Link
                    to={`/blockdetailpage/${stablecoin?.number}`}
                    className="w-[12%] text-dark-red pl-6 "
                  >
                    <p>{stablecoin?.number && stablecoin?.number}</p>
                  </Link>

                  <p className="w-[10%] text-center">
                    {stablecoin?.timestamp &&
                      secondsAgo(
                        stablecoin?.timestamp && stablecoin?.timestamp
                      )}
                  </p>

                  <Link
                    to={`/producerdetailpage/${stablecoin?.witnessAddress}`}
                  ...
                  <p className="w-[12%] text-center">
                    {stablecoin?.nrOfTrx && stablecoin?.nrOfTrx}
                  </p>
                  <p className="w-[16%] text-center ">
                    {stablecoin?.energyUsage && stablecoin?.energyUsage}
                  </p>
                  <p className="w-[12%] text-center ">
                    {stablecoin?.netUsage && stablecoin?.netUsage}
                  </p>
                  <p className="w-[12%] text-center  ">
                    {stablecoin?.blockReward && stablecoin?.blockReward} POX
                  </p>
                  <p className="w-[14%] text-center text-dark-green">
                    {stablecoin?.confirmed && stablecoin?.confirmed
                      ? "CONFIRMED"
                      : "UNCONFIRMED"}
                  </p>
                </div>
