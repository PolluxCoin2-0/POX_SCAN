...
import {
  ...
  getTransferTableDataOfPoxTransfer,
  getTransferTableDataOfPRC20Transfer,
  ...
  return (
    <div>
      <div className="bg-white rounded-2xl p-4 md:p-10 overflow-x-auto lg:overflow-x-auto xl:overflow-x-auto 2xl:overflow-hidden">
        ...
        {data?.transactions &&
          data?.transactions.map((contact, index) => {
            return (
              <>
                <div className="min-w-[1300px] flex flex-row justify-evenly items-center p-5 border-b-2 border-lightest-gray  rounded-xl pr-0">
                  <Link
                    to={`/transactiondetails/${contact?.transactionId}`}
                    className="whitespace-nowrap w-[18%] text-center "
                  >
                    <p>
                      <span className="text-dark-red px-2">
                        {contact?.transactionId &&
                          shortenString(contact?.transactionId, 10)}
                      </span>
                    </p>
                  </Link>

                  <Link
                    to={`/blockdetailpage/${contact?.blockNumber}`}
                    className="w-[8%] text-dark-red text-center"
                  >
                    <p>{contact?.blockNumber && contact?.blockNumber}</p>
                  </Link>

                  <p className="w-[12%]  text-center  ">
                    {contact?.timeStamp && secondsAgo(contact?.timeStamp)}
                  </p>
                  <p className="w-[12%]  text-center  ">
                    {contact?.assetName && contact?.assetName}
                  </p>

                  <Link
                    to={`/accountdetails/${contact?.fromAddress}`}
                    className="w-[14%] text-center text-dark-red"
                  >
                    <p>
                      {contact?.fromAddress &&
                        shortenString(contact?.fromAddress, 5)}
                    </p>
                  </Link>

                  <Link
                    to={`/tokendetailpage/${contact?.toAddress}`}
                    className="w-[14%] text-center text-dark-red"
                  >
                    <p>
                      {contact?.toAddress &&
                        shortenString(contact?.toAddress, 5)}
                    </p>
                  </Link>

                  <p className="w-[12%] text-center  ">
                    {Number(contact?.assetAmount / Math.pow(10, 6)).toFixed(6)}{" "}
                    {contact?.assetName && contact?.assetName}
                  </p>
              ...
              <>
                <div className="min-w-[1300px] flex flex-row justify-evenly items-center p-5 border-b-2 border-lightest-gray  rounded-xl pr-0">
                  <div className="whitespace-nowrap text-center w-[16%] ">
                    <div className="flex justify-center space-x-4 mb-1">
                      <p className="text-dark-red ">
                        {" "}
                        Pollux...({contact?.assetName && contact?.assetName})
                      </p>
                      <p className="px-2 py-[1px] t rounded-md bg-light-red text-black">
                        {" "}
                  ...
                  >
                    <p>
                      {contact?.fromAddress &&
                        shortenString(contact?.fromAddress, 4)}
                    </p>
                  </Link>
                  ...
                  >
                    <p>
                      {contact?.toAddress &&
                        shortenString(contact?.toAddress, 4)}
                    </p>
                  </Link>
                  ...
                  >
                    <p>
                      {contact?.transactionId &&
                        shortenString(contact?.transactionId, 5)}
                    </p>
                  </Link>
  ...
  return (
    <div className="px-4 md:px-4 lg:px-4 xl:px-12 2xl:px-12 pb-12">
      ...
      <div className="">
        ...
        <div
          className=" flex flex-col space-y-6 lg:space-y-0 xl:space-y-0 2xl:space-y-0 md:space-y-6 md:flex-col lg:flex-row xl:flex-row 2xl:flex-row
        ...
        >
              ...
              <div className=" flex flex-row justify-between rounded-lg py-5">
                <div className="">
                  <p className="text-xl font-bold pt-5">
                    {data?.totalPage && formatNumberWithCommas(data?.totalPage)}
                  </p>
                  <p className="text-light-gray  pt-2">Total</p>
                </div>

                <div className="text-right">
                  <p className="text-dark-green font-bold text-xl pt-5">0</p>
              ...
              <div className=" flex flex-row justify-between  rounded-lg pt-6 pb-6">
                  ...
                  <p className="text-xl font-bold pt-5">
                    {stats?.Total_Volume &&
                      formatNumberWithCommas(
                        Number(stats?.Total_Volume / Math.pow(10, 6)).toFixed(2)
                      )}
                  </p>
                ...
                <div className="text-right">
                  <p className="text-xl font-bold text-dark-green pt-5">
                    {stats?.Yesterday_Volume &&
                      formatNumberWithCommas(
                        Number(
                          stats?.Yesterday_Volume / Math.pow(10, 4)
                        ).toFixed(2)
                      )}
                    b
                  </p>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full md:w-full lg:w-[48%] xl:w-[48%] 2xl:w-[48%] py-3">
            ...
            <div
              ...
              md:space-y-0 md:flex-row md:items-center md:justify-evenly
            lg:flex-col 
            xl:flex-col 2xl:flex-row 2xl:justify-between
            "
            >
              <div className="lg:h-64 xl:h-64 lg:ml-12 xl:ml-12 h-80 lg:w-full xl:w-full 2xl:w-[70%]">
