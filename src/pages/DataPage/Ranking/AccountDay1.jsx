  ...
  <div className="bg-white rounded-xl pt-6 pl-10 pr-10 pb-10">
    ...
    {amountTableData?.map &&
      amountTableData?.map((param, index) => {
        return (
          <div key={index} className="flex flex-row justify-around p-2 border-b-2 border-b-lightest-gray pr-0">
      ...
      tableDataArray?.map((param, index) => {
        return (
          <div key={index} className="flex flex-row justify-around p-5 border-b-2 border-b-lightest-gray">
            ...
            <Link to={`/accountdetails/${param?.fromAddress}`}>
            <p className="w-[40%] text-dark-red">{param?.fromAddress && shortenString(param?.fromAddress, 10)}</p>
            </Link>
