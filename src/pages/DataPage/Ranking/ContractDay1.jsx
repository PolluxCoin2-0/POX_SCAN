...
import { getRankingContractDayOneData } from "../../../utils/axios/Data";
...
const TableData = ({title, tableDataArray, type}) => {
    ...
    useEffect(() => {
      ...
      const fetchData = async () => {
        try {
          const data = await getRankingContractDayOneData();
          console.log(data);
          setData(data);
    ...
    {tableDataArray?.map && tableDataArray?.map((param, index) => {
      ...
      return (
        <div key={index} className="flex flex-row justify-around p-5 border-b-2 border-b-lightest-gray">
          ...
          <Link to={`/accountdetails/${param?.contractAddress}`} className="w-[60%] text-center text-dark-red">
