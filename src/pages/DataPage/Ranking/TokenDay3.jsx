...
import { getRankingTokenDayOneData, getRankingTokenDayThreeData } from "../../../utils/axios/Data";
...
const HolderTable= ({title}) => {
    ...
    useEffect(() => {
      ...
      const fetchData = async () => {
        try {
          const data = await getRankingTokenDayThreeData();
          console.log(data);
          setData(data);
  ...
  const VolumeTable = ({title}) => {
    ...
    useEffect(() => {
      ...
      const fetchData = async () => {
        try {
          const data1 = await getRankingTokenDayThreeData();
          ...
          {data1?.topTokensbyVol?.map && data1?.topTokensbyVol?.map((param, index) => {
            ...
            return (
              <div key={index} className="flex flex-row justify-around p-5 border-b-2 border-b-lightest-gray">
                ...
                <p className="w-[10%] text-center ">{param?.count}</p>
                <p className="w-[20%] text-center ">{param?.volume.toFixed(2)}%</p>
              </div>
            );
          })}
        </div>
