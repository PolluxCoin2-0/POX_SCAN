...
import {
  getPriceChartGraphData,
  getResourceDetailsData,
  getStakedData,
  getTrendingSearchData,
  getTrendingSearchGraphData,
  getTvlPriceData,
      ...
      <div className="flex flex-row items-center space-x-4">
        ...
        <div>
          ...
          <p className="font-semibold">
            {title === "Total Transfer Volume" ? "$" : ""} {value && value}
          </p>
        </div>
      </div>
        ...
        <div className="shadow-md px-2 mb-6 rounded-xl py-6 bg-white flex flex-col md:flex-row justify-evenly">
          <div className="flex flex-col w-full  md:w-[40%] px-2 md:px-0">
            <div className="mb-6 border-b-[2px] border-text-bg-gray pb-6">
              <CardForTrendingSearch
                ...
                value={
                  data?.totaladdr &&
                  formatNumberWithCommas(parseInt(data?.totaladdr))
                }
                valueFor24hr={
                  data?.last24addr &&
                  formatNumberWithCommas(parseInt(data?.last24addr))
                }
              />
            </div>
            <div className="pb-4 md:pb-0 mb-6 md:mb-0 border-b-[2px] md:border-none border-text-bg-gray">
              <CardForTrendingSearch
                ...
                value={
                  data?.totaltxn &&
                  formatNumberWithCommas(parseInt(data?.totaltxn))
                }
                valueFor24hr={
                  data?.last24hourtxn &&
                  formatNumberWithCommas(parseInt(data?.last24hourtxn))
                }
              />
            </div>
          </div>
          ...
          <div className="flex flex-col justify-around w-full md:w-[40%] px-2 md:px-0">
            ...
            <CardForTrendingSearch
              ...
              value={
                data3?.tradingvol &&
                formatNumberWithCommas(parseInt(data3?.tradingvol))
              }
              valueFor24hr={""}
            />
          </div>
        ...
        <div className="shadow-lg bg-white px-4 py-6 rounded-xl">
          <div className="flex space-x-4 items-center">
            ...
            <p className="font-medium text-light-gray">
              POX{" "}
              <span className="text-darker-blue font-bold">
                ${data3?.price}
              </span>
            </p>
          </div>
            ...
            <div className="flex justify-between">
              <div>
                <p className="text-light-gray mb-1">
                  Market Cap:
                  <span className="text-darker-blue font-semibold">
                    {" "}
                    $
                    {data3?.totalmatket &&
                      (data3?.totalmatket / Math.pow(10, 6)).toFixed(2)}
                    M
                  </span>
                </p>
                <p className="text-light-gray">
                  ...
                  <span className="text-darker-blue font-semibold">
                    {" "}
                    {data?.last24hourvol &&
                      (data?.last24hourvol / Math.pow(10, 3)).toFixed(2)}
                    K
                  </span>
                </p>
              </div>
              <div>
                <p className="text-light-gray mb-1">
                  ...
                  <span className="text-darker-blue font-semibold">
                    {" "}
                    {data3?.circsupply && data3?.circsupply} POX
                  </span>
                </p>
                <p className="text-light-gray whitespace-nowrap">
                  Staked:
                  <span className="text-darker-blue font-semibold">
                    {" "}
                    {data6?.totalTvl && data6?.totalTvl}
                  </span>
                </p>
              </div>
            </div>
